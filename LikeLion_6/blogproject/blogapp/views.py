...
def create_comment(request, blog_id):
    ...
    if filled_form.is_valid():
        finished_form = filled_form.save(commit=False) #필드폼을 일단은 저장하지 말고 대기해
        finished_form.post = get_object_or_404(Blog, pk=blog_id) #어떤 게시물에 달린 댓글인지 블로그값에다가 pk값이 id인걸로 담아줌
        finished_form.save()
